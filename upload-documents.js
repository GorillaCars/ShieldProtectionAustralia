(function () {
  ...
  function friendlyUploadError(error) {
    ...
    if (message.includes("policy details") || message.includes("do not match")) {
      return "Those details do not match a policy. Please check the product number and full name exactly as written on your document.";
    }
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (!policyNo || !fullName || !(file instanceof File) || file.size === 0) {
      setStatus("Please enter your product details and choose a file.", "error");
      return;
    }
    ...
    try {
      ...
      if (intentError || !intent?.[0]?.upload_token) {
        throw new Error("Those details do not match a policy. Please check the product number and full name exactly as written on your document.");
      }
